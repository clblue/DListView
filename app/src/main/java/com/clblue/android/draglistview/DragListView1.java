...
public class DragListView1 extends ListView {
    ...
    public DragListView1(Context context) {
        this(context, null);
    }
    ...
    public DragListView1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN: // 判断是否进拖拽
                ...
                if (itemView != null && mDragItemListener != null && mDragItemListener.canDrag(itemView,
                        mDownX, mDownY)) {
    ...
    public void stopDrag() {
        if (mBitmap != null) {
            ...
            if (mDragItemListener != null) {
                mDragItemListener.onRelease(mCurrentPosition, mItemView, mLastY -
                        mDragViewOffset, mLastX, mLastY);
            }
        }
        if (mItemView != null) {
            mItemView = null;
    ...
    private void checkExchange(int y) {
        if (mCurrentPosition != mLastPosition) { // // 数据交换
            if (mDragItemListener != null) {
                if (mDragItemListener.canExchange(mLastPosition, mCurrentPosition)) { // 进行数据交换,
                    // true则表示交换成功
                    View lastView = mItemView;
                    mItemView = getChildAt(mCurrentPosition - getFirstVisiblePosition());
                    // 通知交换数据成功，可在此时设置交换的动画效果
                    mDragItemListener.onExchange(mLastPosition, mCurrentPosition, lastView,
                            mItemView);
    ...
    public static abstract class SimpleAnimationDragItemListener implements DragItemListener {
        @Override
        public void onRelease(int positon, View itemView, int itemViewY, int releaseX, int
                releaseY) {
            itemView.setVisibility(View.VISIBLE);
            if (itemView != null && Math.abs(itemViewY - itemView.getTop()) > itemView.getHeight
                    () / 5) {
        ...
        public void onExchange(int srcPosition, int position, View srcItemView, View itemView) {
            if (srcItemView != null) {
                int height = srcPosition > position ? -srcItemView.getHeight() : srcItemView
                        .getHeight();
